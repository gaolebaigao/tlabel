...
### v0.9.0 — 交互式面板 Phase 1 + 导出插件注册中心
**专业级触觉标注体验：丰富的可视化 + 可扩展的导出管线。**
- 🎨 **伪 GelSight 可视化**：实时触觉接触热力图，径向渐变 + 力度环
- ⌨️ **快捷键标注**：空格（切换标注）、←→（帧导航）、↑↓（标签调节）
- 📊 **时间轴框选**：点击跳转 + 拖拽选择帧范围，批量编辑
- 🤖 **AI 预标注**：一键自动标注 + 置信度阈值控制
- 📦 **统一导出中心**：一个 Tab 导出所有格式 — JSON, CSV, HDF5, FTP-1, LeRobot, RLDS(stub), ROS2(stub)
- 🔌 **导出插件注册中心**：`ExporterBase` 基类 + 动态 `register()`/`unregister()` — 新格式只需写子类
