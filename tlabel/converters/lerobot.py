...
def tlabel_to_lerobot(
    ...
    # 构建触觉特征矩阵
    # 注意: schema_v2 中部分字段为向量(list)或枚举(str)，float() 无法直接转换，
    # 需要安全转换：向量取模、枚举/bool 转 0/1、None 转 0.0
    def _safe_float(val) -> float:
        ...
        if isinstance(val, (list, tuple, np.ndarray)):
            # 向量字段取 L2 范数（模长）作为标量特征
            try:
                arr = np.array(val, dtype=float)
                return float(np.linalg.norm(arr)) if arr.size > 0 else 0.0
            except (ValueError, TypeError):
                return 0.0
        if isinstance(val, str):
            # 枚举字段：非空字符串视为 1.0，空字符串视为 0.0
            return 1.0 if val else 0.0
        return 0.0
    ...
    for frame in frames:
        ...
        row = [_safe_float(sv2.get(name, 0.0)) for name in feature_names]
